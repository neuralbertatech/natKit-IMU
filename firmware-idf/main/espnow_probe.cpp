...
void sweepPayloadSizes() {
  ...
  for (size_t size : kSizes) {
    ...
    memcpy(buffer, &size, sizeof(uint32_t));  // sequence slot, harmless here
...
void runEspNowProbe() {
  ...
  while (true) {
    ...
    ESP_LOGI(kTag,
             "recv %lu packets (+%lu/s), %lu bytes, seq gaps %lu, last seq %lu",
  ...
  ESP_LOGI(kTag,
           "probe done; idling. Flash a second board with "
           "CONFIG_NATKIT_ESPNOW_PROBE_RECEIVER=y to measure loss.");
  while (true) {
